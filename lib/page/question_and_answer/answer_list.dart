...
import 'package:ksp/widget/image.dart';
...
class AnswerFormState extends State<AnswerForm> {
  ...
  @override
  void initState() {
    _futureModel = postDio(answerApi + 'read', {
      'limit': _limit,
      'skip': 0,
          ...
          itemBuilder:
              (context, index) => InkWell(
                ...
                child: Container(
                  ...
                  child: Image.network(gallerys[index]['imageUrl']),
                ),
              ),
        ),
      ),
      ...
      child: Column(
        children: [
          Row(
            ...
            children: [
              Text('Q:', style: TextStyle(fontFamily: 'Kanit', fontSize: 40.0)),
              Expanded(
                child: Container(
  ...
  Container cardAnswer(dynamic model, int index, bool isShowReply) {
    var image =
        model['imageUrlCreateBy'] != null ? model['imageUrlCreateBy'] : '';
    var title = model['title'];
    var createBy = model['createBy'];
    return Container(
      ...
      child: Column(
        children: [
          ...
          Row(
            children: [
              SizedBox(width: 46.0),
              imageCircle(context, image: image, width: 30.0, height: 30.0),
              SizedBox(width: 5.0),
              Text(
                createBy,
                style: TextStyle(fontFamily: 'Kanit', fontSize: 10.0),
          ...
          isShowReply
              ? Row(
                children: [
                  Expanded(child: rowReply(_controllers[index], model, index)),
                ],
              )
              : Container(),
        ],
  ...
  getStorage() async {
    ...
    var code = await storage.read(key: 'profileCode9') ?? "";
  ...
  delete() {
    try {
      postDio(questionApi + 'delete', widget.question).then(
        (value) => {
          print(value),
          Navigator.of(context).pop(),
          Navigator.of(context).pop(),
